...
describe("test txpool", () => {
    ...
    test("should throw error when secret is invalid", async () => {
      try {
        await txpool.batchSignWithSeqs({txList: [{
          ...
          TransactionType: "Payment"
        }], seqs: [123], secret: "fjeslfjkrirgrlew3"});
      } catch (error) {
        expect(error.message).toEqual("Secret is invalid");
      }
    });
    ...
    test("should throw error when when parameters are invalid", async () => {
      await expect(
        txpool.submitToTxPool({
          uuid: "", publicKey: "", submitPara: { dataHashSign: "FJekfjke", dataJsonStr: "FJekfjke" }
        })
      ).rejects.toThrow(new Error("PublicKey is invalid"));
      await expect(
        txpool.submitToTxPool({
          uuid: "", publicKey: "FJekfjke", submitPara: { dataHashSign: "", dataJsonStr: "FJekfjke" }
        })
      ).rejects.toThrow(new Error("DataHashSign is invalid"));
      await expect(
        txpool.submitToTxPool({
          uuid: "", publicKey: "FJekfjke", submitPara: { dataHashSign: "FJekfjke", dataJsonStr: "" }
        })
    ...
    test("should throw error when when parameters are invalid", async () => {
      await expect(
        txpool.fetchSubmittedData({
          uuid: "", publicKey: "", state: 1, count: "one"
        })
      ).rejects.toThrow(new Error("PublicKey is invalid"));
      await expect(
        txpool.fetchSubmittedData({
          uuid: "", publicKey: "jf2k2lj3jfl", state: 0, count: "all"
        })
      ).rejects.toThrow(new Error("State is invalid"));
      await expect(
        txpool.fetchSubmittedData({
          uuid: "", publicKey: "jf2k2lj3jfl", state: 1, count: "fsef"
        })
      ).rejects.toThrow(new Error("Count is invalid"));
    });
    ...
    test("should throw error when response is not success", async () => {
      ...
      try {
        await txpool.fetchSubmittedData({
          uuid: "", publicKey: "eiofjel", state: 2, count: "one"
        });
      } catch (error) {
        expect(error instanceof CloudError).toEqual(true);
        expect(error.code).toEqual("-1");
    ...
    test("should return submitted list", async () => {
      stub.resolves({
        ...
        data: [
          {
            id: 1287505,
            txSign: '',
          ...
          {
            id: 1287506,
            txSign: '',
  ...
  describe("test cancelSubmitChain", () => {
    ...
    test("should throw error when when parameters are invalid", async () => {
      await expect(
        txpool.cancelSubmitChain({ uuid: "", publicKey: "", signedAddr: "sfsf" })
      ).rejects.toThrow(new Error("PublicKey is invalid"));
      await expect(
        txpool.cancelSubmitChain({ uuid: "", publicKey: "eiofjel", signedAddr: "" })
      ).rejects.toThrow(new Error("SignedAddr is invalid"));
    });
    ...
    test("should throw error when response is not success", async () => {
      ...
      await expect(
        txpool.cancelSubmitChain({ uuid: "", publicKey: "eiofjel", signedAddr: "sfsf" })
      ).rejects.toThrow(new CloudError("-1", "error"));
    });
