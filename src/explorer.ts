...
import { IResponse, OrderType, TradeType, PageSize,
  ...
  IFetchBlockTransactionsOptions, IFetchBlockTransactionsResponse,
  IFetchLatestSixBlocksOptions, IFetchLatestSixBlocksResponse,
  IFetchAllBlocksOptions, IFetchAllBlocksResponse
...
export default class JCCDexExplorer {
  ...
  private isSuccess(code: string): boolean {
    return code === "0";
  }
  ...
  public async fetchBlockTransactions(options: IFetchBlockTransactionsOptions): Promise<IFetchBlockTransactionsResponse> {
    ...
    const transactions = (data.list as any[] || []);
